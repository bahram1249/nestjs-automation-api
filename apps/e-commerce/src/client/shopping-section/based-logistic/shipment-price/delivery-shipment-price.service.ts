...
import { ECAddress, ECDiscount, ECDiscountType } from '@rahino/localdatabase/models';
  ...
  async cal(
    ...
    if (!addressId) {
      return {
        ...
        typeName: this.localizationService.translate('ecommerce.shipment.delivery' as any) as unknown as string,
        price: 0,
        realShipmentPrice: 0,
      };
    }
    ...
    const freeShipmentByItems = stocks.length > 0 && stocks.every((s) => !!s.freeShipment);
    ...
    const priceBase = Number(baseCourierPrice?.value || 0) + km * Number(courierPriceByKilometre?.value || 0);
