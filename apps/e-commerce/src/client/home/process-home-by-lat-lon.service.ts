...
export class ProcessHomeByLatLonService {
  ...
  async processAll(dto: ProcessHomeByLatLonDto) {
    ...
    for (let index = 0; index < items.length; index++) {
      ...
      const processedItem = await this.processItem(dto, item);
  ...
  async processItem(dto: ProcessHomeByLatLonDto, item: ECHomePage) {
  ...
  async processProductCategory(
    ...
    return {
      ...
      link:
        baseUrl +
        `/v1/api/ecommerce/products/byVendorNearby?entityTypeId=${input.entityTypeId}&orderBy=${sort.sortField}&sortOrder=${sort.sortOrder}&latitude=${latLon.latitude}&longitude=${latLon.longitude}`,
      totalLink: frontUrl + `/category/${entityType.slug}`,
      requestBased: true,
    };
  }

  async processProductBrand(
    priority: number,
    input: ProductBrandContentDto,
    latLon: ProcessHomeByLatLonDto,
  ) {
    const baseUrl = this.config.get('BASE_URL');
    const frontUrl = this.config.get('BASE_FRONT_URL');
    ...
    return {
      ...
      link:
        baseUrl +
        `/v1/api/ecommerce/products/byVendorNearby?brands=${input.brandId}&orderBy=${sort.sortField}&sortOrder=${sort.sortOrder}&latitude=${latLon.latitude}&longitude=${latLon.longitude}`,
      totalLink: frontUrl + `/brand/${brand.slug}`,
      requestBased: true,
    };
  }

  async processAmazing(
    priority: number,
    input: AmazingContentDto,
    latLon: ProcessHomeByLatLonDto,
  ) {
    const baseUrl = this.config.get('BASE_URL');
    const frontUrl = this.config.get('BASE_FRONT_URL');
    ...
    return {
      ...
      link:
        baseUrl +
        `/v1/api/ecommerce/products/byVendorNearby?discountTypeId=${this.amazingTypeId.toString()}&orderBy=${
          sort.sortField
        }&sortOrder=${sort.sortOrder}&latitude=${latLon.latitude}&longitude=${
          latLon.longitude
        }`,
