...
export class BasedAdminPostService {
  ...
  private buildVendorExistsClause(filter: GetBasedAdminPostDto) {
  ...
  async findAll(filter: GetBasedAdminPostDto) {
    ...
    if (filter.vendorId) {
      // join details (vendor-scoped) and aggregate per group
      qb = qb.thenInclude({
        attributes: [],
  ...
  async total(filter: GetBasedAdminPostDto) {
    const isValidBeginDate = await this.isValidDate(filter.beginDate);
    if (!isValidBeginDate) {
      throw new BadRequestException(
        this.i18n.t('ecommerce.date_is_invalid', { lang: I18nContext.current().lang }),
