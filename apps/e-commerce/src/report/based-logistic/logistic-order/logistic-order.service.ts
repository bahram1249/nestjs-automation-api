...
@Injectable()
export class LogisticOrderService {
  constructor(
  ...
  private buildVendorExistsClause(filter: GetLogisticOrderDto) {
    if (!filter.vendorId) return null;
    const sql = `
      EXISTS (
        SELECT 1
         ...
         WHERE GD.groupedId = ECLogisticOrderGrouped.id
           ...
           AND GD.vendorId = ${Number(filter.vendorId)}
      )
    `.replace(/\s\s+/g, ' ');
    return Sequelize.literal(sql);
  }

  async findAll(filter: GetLogisticOrderDto) {
    ...
    // Build base filters on group with parent order date range
    const vendorExists = this.buildVendorExistsClause(filter);

    // Count groups
    let countQB = new QueryOptionsBuilder().include([]);
    // include parent order to filter by date and parent order status != WaitingForPayment
    ...
    // filter by date range on parent order
    countQB = countQB
      .filter({
        // parent order date range
        // logisticOrder.gregorianAtPersian >= beginDate
        // using Sequelize.where on included alias
        // beginDate
        // below two filters will be combined by AND
      })
      .filter(
        Sequelize.where(
          Sequelize.col('logisticOrder.gregorianAtPersian'),
          { [Op.gte]: filter.beginDate },
        ) as any,
      )
      .filter(
        Sequelize.where(
      ...
      .filter(
        Sequelize.where(
          Sequelize.col('logisticOrder.orderStatusId'),
          { [Op.ne]: OrderStatusEnum.WaitingForPayment },
        ) as any,
      );

    // group-level filters
    if (filter.orderShipmentWayId) {
      countQB = countQB.filter({ orderShipmentWayId: filter.orderShipmentWayId });
    }
    if (filter.logisticId) {
      countQB = countQB.filter({ logisticId: filter.logisticId });
    }
    if (filter.courierId) {
      countQB = countQB.filter({ courierUserId: filter.courierId as any });
    }
    if (filter.orderId) {
    ...
    if (vendorExists) {
      countQB = countQB.filter(vendorExists as any);
    }
    ...
    if (filter.orderShipmentWayId) {
      qb = qb.filter({ orderShipmentWayId: filter.orderShipmentWayId });
    }
    if (filter.logisticId) {
      qb = qb.filter({ logisticId: filter.logisticId });
    }
    if (filter.courierId) {
      qb = qb.filter({ courierUserId: filter.courierId as any });
    }
    if (filter.orderId) {
      qb = qb.filter(
        Sequelize.where(Sequelize.col('logisticOrder.id'), { [Op.eq]: filter.orderId }) as any,
    ...
    if (!filter.vendorId && vendorExists) {
      qb = qb.filter(vendorExists as any);
    }

    if ((filter as any).vendorId) {
      // Join vendor details and aggregate per group
      qb = qb.thenInclude({
        attributes: [],
        ...
        required: true,
        where: {
          vendorId: (filter as any).vendorId as any,
          isDeleted: { [Op.eq]: 0 } as any,
        },
      } as any);
  ...
  async total(filter: GetLogisticOrderDto) {
    const isValidBeginDate = await this.isValidDate(filter.beginDate);
    if (!isValidBeginDate) {
      throw new BadRequestException(
        this.i18n.t('ecommerce.date_is_invalid', { lang: I18nContext.current().lang }),
    ...
    if ((filter as any).vendorId) {
      // join details and aggregate all values across details rows
      qb = qb.thenInclude({
        attributes: [],
        model: ECLogisticOrderGroupedDetail,
        as: 'details',
      ...
      qb = qb
        .attributes([
          ...
          [
            Sequelize.fn(
              'isnull',
              Sequelize.literal('sum(isnull(ECLogisticOrderGrouped.realShipmentPrice, 0))'),
              0,
            ),
            'realShipmentPrice',
          ],
          [
            Sequelize.fn(
              'isnull',
              Sequelize.literal('sum(isnull(ECLogisticOrderGrouped.shipmentPrice, 0))'),
              0,
            ),
            'totalShipmentPrice',
          ],
          ...
          [
            Sequelize.fn(
              'isnull',
              Sequelize.literal('sum(isnull(ECLogisticOrderGrouped.totalProductPrice, 0))'),
              0,
            ),
            'totalProductPrice',
          ],
          [
            Sequelize.fn(
              'isnull',
              Sequelize.literal('sum(isnull(ECLogisticOrderGrouped.totalDiscountFee, 0))'),
              0,
            ),
            'totalDiscountFee',
          ],
          [
            Sequelize.fn(
              'isnull',
              Sequelize.literal('sum(isnull(ECLogisticOrderGrouped.totalPrice, 0))'),
              0,
            ),
            'totalPrice',
          ],
    ...
    // Remove pagination and ordering for aggregation
