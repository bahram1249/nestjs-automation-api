...
export class BasedAdminCourierService {
  ...
  private buildVendorExistsClause(filter: GetBasedAdminCourierDto) {
  ...
  async findAll(filter: GetBasedAdminCourierDto) {
    ...
    if (filter.logisticId) countQB = countQB.filter({ logisticId: filter.logisticId });
    if (filter.courierId) countQB = countQB.filter({ courierUserId: filter.courierId as any });
    if (filter.orderId)
    ...
    if (filter.logisticId) qb = qb.filter({ logisticId: filter.logisticId });
    if (filter.courierId) qb = qb.filter({ courierUserId: filter.courierId as any });
    if (filter.orderId)
      qb = qb.filter(
        Sequelize.where(Sequelize.col('logisticOrder.id'), { [Op.eq]: filter.orderId }) as any,
      );
    ...
    // build attributes
    if (filter.vendorId) {
      // join details and aggregate per group
      qb = qb.thenInclude({
        attributes: [],
        ...
        required: true,
        where: {
          vendorId: filter.vendorId as any,
  ...
  async total(filter: GetBasedAdminCourierDto) {
    const isValidBeginDate = await this.isValidDate(filter.beginDate);
    if (!isValidBeginDate) {
      throw new BadRequestException(
        this.i18n.t('ecommerce.date_is_invalid', { lang: I18nContext.current().lang }),
    ...
    if (filter.vendorId) {
      // aggregate across details rows (vendor-scoped)
      qb = qb.thenInclude({
        attributes: [],
        model: ECLogisticOrderGroupedDetail,
        as: 'details',
