...
export class LogisticSaleQueryBuilderService {
  ...
  init(groupByQuery: boolean) {
    ...
    // Ensure we join through grouped -> logisticOrder so we can filter on order state
  ...
  addBeginDate(beginDate: string) {
    // filter on detail gregorianAtPersian (creation time of detail rows)
    this.builder = this.builder.filter({
      gregorianAtPersian: { [Op.gte]: beginDate },
    });
    return this;
  }
