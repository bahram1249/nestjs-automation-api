...
@Injectable()
export class NotificationSenderForClienSupplierStateRequestActionService
  implements ActionServiceImp
{
  constructor(
    ...
    @InjectQueue(CLIENT_SUPPLIER_STATE_REQUEST_SMS_SENDER_QUEUE)
  ...
  private async sendNotification(user: User) {
    await this.clientSmsSenderQueue.add(
      'send-client-supplier-state-request-sms',
      {
        id: user.id,
