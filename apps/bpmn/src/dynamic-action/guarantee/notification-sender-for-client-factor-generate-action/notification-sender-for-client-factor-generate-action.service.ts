...
@Injectable()
export class NotificationSenderForClientFactorGenerateActionService
  implements ActionServiceImp
{
  constructor(
    @InjectModel(GSRequest)
    private readonly requestRepository: typeof GSRequest,
    @InjectQueue(CLIENT_FACTOR_GENERATE_SMS_SENDER_QUEUE)
    private readonly clientFactorGenerateQueue: Queue,
  ) {}
  ...
  private async sendNotification(user: User) {
    await this.clientFactorGenerateQueue.add(
      'send-client-factor-generate-sms',
      {
        id: user.id,
